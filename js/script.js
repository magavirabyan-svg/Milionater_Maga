...
let count = 0; let incorrectSoundFlag = false;
generalMusic.loop = true; window.addEventListener('click', () => { generalMusic.play(); }, { once: true });
...
endBtn.addEventListener('click', () => {
  ...
  setTimeout(() => { game.computedStyleMap.backgroundImage = "" },)
  questionSong.pause()
  ...
  // setTimeout(() => { game.style.backgroundImage = ""; }, 2000); 
  // setTimeout(() => { startBtn.classList.remove('animate__backInDown'); }, 2000); 
  ...
  winDiv.className = '.user-win animate-animated animate-fedIn'
  winDiv.style.cssText = 'text-align:center; font-size: color: white; margin-top: 300px;';
  startBtn.insertAdjacentElement('afterend', winDiv)
  setTimeout(() => {
    winDiv.classList.replace('animate_fadeIm', 'animate_fadeOut')
...
btnAnswers.forEach((btnAnswers) => {
  ...
  btnAnswers.btnAnswers.addEventListener('click', (e) => {
